...
app.get('/admin/charities/:id', requireAuth, async (req: any, res: Response) => {
  try {
    await assertAdmin(req, res)
    const id = Number(req.params.id)
    if (!Number.isFinite(id)) return res.status(400).json({ error: 'invalid_id' })
    const pool = getPool()
    const [[charity]]: any = await pool.query('SELECT * FROM charity WHERE id = ?', [id])
    if (!charity) return res.status(404).json({ error: 'not_found' })
    const [typesRows]: any = await pool.query(
      'SELECT t.name FROM charity_type ct JOIN types t ON t.id = ct.type_id WHERE ct.charity_id = ?',
      [id]
    )
    const [itemsRows]: any = await pool.query(
      'SELECT i.name FROM charity_item ci JOIN item i ON i.id = ci.item_id WHERE ci.charity_id = ?',
      [id]
    )
    res.json({
      charity,
...
app.put('/admin/charities/:id', requireAuth, async (req: any, res: Response) => {
  try {
    await assertAdmin(req, res)
    const id = Number(req.params.id)
    if (!Number.isFinite(id)) return res.status(400).json({ error: 'invalid_id' })
    const pool = getPool()
    const [[existing]]: any = await pool.query('SELECT id FROM charity WHERE id = ?', [id])
